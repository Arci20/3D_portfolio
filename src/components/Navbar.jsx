    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary `}>
     ...
     <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
      ...
      window.scrollTo(0,0);}}>
        ...
        <p className='text-white text-[18px] font-bold cursor-pointer flex'>Arci | &nbsp; <span className='sm:block hidden'>Developer</span></p>
      </Link>
      ...
      <div className='sm:hidden flex flex-1 justify-end items-center'>
        ...
        <div className={`${!toggle ? 'hidden' : 'flex'} p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-x`}>
        ...
        {navLinks.map((Link) => (
          ...
          onClick={() => {
            setToggle(!toggle);
             setActive(Link.title);}}> <a href={`#${Link.id}`}>{Link.title}</a></li>
        ))}
